...
//   ChevronDown, ChevronUp, RefreshCw, MapPin,
//   Building2, CalendarDays, Wallet, CircleDot,
//   CheckCircle2, XCircle, Beaker, ClipboardCheck,
//   BarChart3, Construction
// } from "lucide-react";
...
//   if (daysLeft !== null && daysLeft < 0) return "critical"; // overdue
...
// const HEALTH_META = {
//   "on-track": { label: "On Track",  bg: "bg-emerald-100", text: "text-emerald-700", dot: "bg-emerald-500", icon: CheckCircle2  },
//   "at-risk":  { label: "At Risk",   bg: "bg-amber-100",   text: "text-amber-700",   dot: "bg-amber-500",   icon: AlertTriangle  },
//   "critical": { label: "Critical",  bg: "bg-red-100",     text: "text-red-700",     dot: "bg-red-500",     icon: ShieldAlert    },
//   "no-data":  { label: "No Reports",bg: "bg-slate-100",   text: "text-slate-500",   dot: "bg-slate-400",   icon: Minus          },
// };
...
//     // ── Step 1: flat projects — NO nested selects (RLS on projects blocks cross-table joins) ──
...
//     // ── Step 2: batch fetch all related tables in parallel ──
//     const [
//       { data: contractors },
//       { data: contracts },
//       { data: issues },
...
//       const projTests   = (tests       || []).filter((t: any) => t.project_id === p.id);
...
//         failed_tests:       projTests.filter((t: any) => t.passed === false).length,
...
//   const noReports = projects.filter(p => getHealth(p) === "no-data").length;
//   const avgProg   = projects.length ? Math.round(projects.reduce((s, p) => s + p.progress, 0) / projects.length) : 0;

//   const tabs: Tab[] = ["All", "Ongoing", "Design Phase", "BOQ Verification"];
...
//                 <Activity size={14} /> Live Project Feed
//               </div>
...
//               {[
//                 { label: "Total",      val: projects.length, cls: "bg-slate-900 text-white" },
//                 { label: "On Track",   val: onTrack,         cls: "bg-emerald-500 text-white" },
//                 { label: "At Risk",    val: atRisk,          cls: "bg-amber-400 text-white" },
//                 { label: "Critical",   val: critical,        cls: "bg-red-500 text-white" },
//                 { label: "Avg Progress", val: `${avgProg}%`, cls: "bg-blue-600 text-white" },
//               ].map(k => (
//                 <div key={k.label} className={`px-5 py-3 rounded-2xl ${k.cls} min-w-[80px] text-center`}>
...
//                 </div>
//               ))}
//               <button onClick={load} className="px-4 py-3 bg-white rounded-2xl border border-slate-200 text-slate-400 hover:text-slate-700 hover:bg-slate-50 transition-all">
...
//                       tab === t ? "bg-slate-900 text-white" : "bg-white border border-slate-200 text-slate-500 hover:border-slate-400"
//                     }`}>
//                     {t} {!loading && <span className="opacity-60 ml-1">({cnt})</span>}
//                   </button>
//                 );
//               })}
//             </div>
...
//               {(["health", "progress", "deadline"] as const).map(s => (
//                 <button key={s} onClick={() => setSortBy(s)}
//                   className={`px-3 py-1.5 rounded-lg capitalize transition-all ${
//                     sortBy === s ? "bg-blue-600 text-white" : "bg-white border border-slate-200 text-slate-500"
//                   }`}>{s}</button>
//               ))}
//             </div>
//           </div>
...
//               <p className="font-black text-slate-400 text-lg">No projects in this category</p>
//             </div>
//           ) : (
...
//                 const HealthIcon = hMeta.icon;

//                 // Budget utilisation (BOQ total vs budget)
...
//                           {/* Health dot */}
//                           <div className={`w-2.5 h-2.5 rounded-full mt-1.5 shrink-0 ${hMeta.dot}`} />
...
//                       {/* Col 2: Progress bars (4 cols) */}
//                       <div className="col-span-12 md:col-span-4 mt-3 md:mt-0 px-0 md:px-6">
//                         <div className="space-y-2">
//                           {/* Physical progress */}
//                           <div>
//                             <div className="flex justify-between items-center mb-1">
//                               <span className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Physical</span>
...
//                             <div>
//                               <div className="flex justify-between items-center mb-1">
//                                 <span className="text-[9px] font-black uppercase text-slate-400 tracking-widest">BOQ vs Budget</span>
//                                 <span className={`text-[10px] font-black ${budgetUtil > 100 ? "text-red-500" : "text-blue-500"}`}>
//                                   {budgetUtil}%
//                                 </span>
//                               </div>
//                               <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
//                                 <div
//                                   className={`h-full rounded-full transition-all ${budgetUtil > 100 ? "bg-red-500" : "bg-blue-400"}`}
...
//                             <CalendarDays size={11} /> No deadline
//                           </div>
//                         )}
...
//                             <ShieldAlert size={10} /> {project.critical_issues} critical
//                           </span>
//                         )}
...
//                             <Beaker size={10} /> {project.failed_tests} test fail
//                           </span>
//                         )}
...
//                             <Minus size={10} /> No reports
//                           </span>
//                         )}
//                       </div>

//                       {/* Col 4: Health + expand (1 col) */}
...
//                         <span className={`flex items-center gap-1 text-[9px] font-black px-2.5 py-1 rounded-full ${hMeta.bg} ${hMeta.text}`}>
//                           <HealthIcon size={9} /> {hMeta.label}
//                         </span>
...
//                             <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Budget & Contract</p>
//                             <div className="space-y-2">
//                               {[
//                                 { label: "Project Budget",   val: project.budget    ? `${fmt(project.budget)} ETB` : "—" },
//                                 { label: "Contract Value",   val: project.signed_amount ? `${fmt(project.signed_amount)} ETB` : "—" },
//                                 { label: "BOQ Total",        val: project.boq_total ? `${fmt(project.boq_total)} ETB` : "—" },
//                                 { label: "Contract Start",   val: project.contract_start ? new Date(project.contract_start).toLocaleDateString("en-GB") : "—" },
//                                 { label: "Contract End",     val: project.contract_end ? new Date(project.contract_end).toLocaleDateString("en-GB") : "—" },
//                               ].map(row => (
...
//                           {/* Site Activity */}
//                           <div className="space-y-4">
//                             <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Site Activity</p>
//                             <div className="space-y-2">
//                               {[
//                                 { icon: ClipboardCheck, label: "Inspections", val: `${project.total_inspections} total, ${project.failed_inspections} failed` },
//                                 { icon: Beaker,         label: "Quality Tests", val: `${project.failed_tests} failed` },
//                                 { icon: ShieldAlert,    label: "Open Issues", val: `${project.open_issues} open${project.critical_issues ? `, ${project.critical_issues} critical` : ""}` },
//                                 { icon: BarChart3,      label: "Daily Reports", val: `${project.report_count} submitted` },
//                                 { icon: CalendarDays,   label: "Last Report", val: project.last_report_date ? new Date(project.last_report_date).toLocaleDateString("en-GB", { day:"2-digit", month:"short", year:"numeric" }) : "None yet" },
//                               ].map(row => (
...
//                             <div className="space-y-2">
//                               <Link
//                                 href={`/admin/project-monitoring/id/report`}
...
//                               </Link>
//                               <Link
//                                 href={`/admin/design-supervision/inspections`}
...
//                               </Link>
//                               <Link
//                                 href={`/admin/design-supervision/issues`}
//                                 className="flex items-center justify-between px-4 py-3 bg-white border border-slate-200 text-slate-700 rounded-xl text-[10px] font-black uppercase hover:border-slate-400 transition-all"
//                               >
//                                 <span>Issues ({project.open_issues} open)</span>
//                                 <ShieldAlert size={13} />
//                               </Link>
//                               <Link
//                                 href={`/admin/design-supervision/quality`}
//                                 className="flex items-center justify-between px-4 py-3 bg-white border border-slate-200 text-slate-700 rounded-xl text-[10px] font-black uppercase hover:border-slate-400 transition-all"
//                               >
...
//                             No daily reports submitted for this project yet. Supervisor should begin submitting site reports.
//                           </div>
//                         )}
...
//                             <ShieldAlert size={14} />
//                             Project is {Math.abs(daysLeft)} days past its deadline. Immediate escalation required.
//                           </div>
//                         )}
//                       </div>
//                     )}
...
//               <p className="text-[9px] font-black uppercase tracking-widest text-slate-300">Health Legend</p>
